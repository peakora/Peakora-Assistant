...
After installing, the user must restart Claude Code for the MCP tools (`mcp__playwright__browser_*`) to appear in this session. If Playwright is unavailable when this skill runs, use the OpenHands capture fallback (below) instead of stopping.
...
## OpenHands capture fallback (no MCP server required)

This skill ships a headless capture script at `scripts/capture.mjs` that uses
the Playwright Node library directly (the same engine the MCP server wraps).
It runs the upstream `references/extract.js` inside a real headless Chromium,
saves the viewport + full-page screenshots, and prints the DOM/CSS snapshot
JSON to stdout. Use it in OpenHands (or any Node environment) when the
Playwright MCP tools are not available:

```bash
# One-time install (run once in a project dir, or globally):
npm install playwright && npx playwright install chromium

# Capture a URL. Prints the snapshot JSON to stdout; writes screenshots
# <domain>-viewport.jpeg and <domain>-fullpage.jpeg to the outdir (or cwd):
node scripts/capture.mjs <url> [outdir] > snapshot.json
```

Then read `snapshot.json` as `domData` and proceed to Phase 2 (Step 1 -
Measure). The screenshot files are the primary visual reference; the JSON is
the precise measurement source. This is the recommended path in OpenHands.
